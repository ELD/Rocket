...
#[cfg(feature = "diesel_async_mysql")]
mod diesel_async_mysql {
    use diesel_async::{AsyncMysqlConnection, AsyncConnection};
    use rocket::figment::Figment;

    use crate::Config;

    #[rocket::async_trait]
    impl crate::Pool for AsyncMysqlConnection {
        type Error = diesel::ConnectionResult<AsyncMysqlConnection>;
        type Connection = AsyncMysqlConnection;
        async fn init(figment: &Figment) -> Result<Self, Self::Error> {
            let config = figment.extract::<Config>()?;
            AsyncMysqlConnection::establish(&config.url).await?;
        }
        async fn get(&self) -> Result<Self::Connection, Self::Error> {}
        async fn close(&self) {}
    }
...
#[cfg(feature = "diesel_async_postgres")]
mod diesel_async_postgres {}
